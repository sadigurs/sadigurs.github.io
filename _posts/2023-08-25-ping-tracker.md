...
I crafted a simple script that periodically sends out ping requests to a server. The responses are logged with timestamps. This simple setup allowed me to collect data on how long it took for these pings to travel to and from the server and store the results in a csv file.
...
I decided to host the dashboard on PythonAnywhere so I could access it anytime. You can check it out [here](http://menisadi.pythonanywhere.com/). It's far from being a masterpiece, but it does the job 😊. 
